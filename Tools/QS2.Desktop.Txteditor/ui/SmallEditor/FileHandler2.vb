...
''' </summary>
Partial Public Class FileHandler2
#Region " Constructor "

    Public Sub New(ByVal mainWindow__1 As System.Windows.Forms.UserControl, ByVal textControl__2 As TXTextControl.TextControl)
        MainWindow = mainWindow__1
        '_fileList = My.Settings.Default.MRUFiles             'lthxy
        '_nMaxFiles = My.Settings.Default.MRUMaxFiles
...
#Region "  Fields  "
    ...
    Const DefaultInsertTypes As TXTextControl.StreamType = TXTextControl.StreamType.All And Not DefaultExportTypes
    Const DefaultLoadTypes As TXTextControl.StreamType = TXTextControl.StreamType.All And Not TXTextControl.StreamType.CascadingStylesheet
...
#Region "  Properties  "
    ...
    Public Property MainWindow() As System.Windows.Forms.UserControl
        Get
            Return m_MainWindow
        End Get
        ...
        End Set
    End Property
    Private m_MainWindow As System.Windows.Forms.UserControl
    ...
    'Public Property RibbonMain() As RibbonLib.Ribbon      'lthxy
    '    Get
    '        Return m_RibbonMain
    '    End Get
    '    Set(ByVal value As RibbonLib.Ribbon)
    '        m_RibbonMain = value
    '    End Set
    'End Property
    'Private m_RibbonMain As RibbonLib.Ribbon
...
#Region "  File methods and functions  "
    ...
    Public Function FileOpen() As Boolean
        ...
        ' (Re)set application mode
        'RibbonMain.SetModes(CByte(FormMain.RibbonApplicationMode.Normal))                'lthxy

        Try
            'if there is no file name, open a dialog to load a document
            If DocumentFileName = "" Then
                'TextControl.Load(DefaultLoadTypes, ls)             'lthxy
            ...
            If ls.LoadedFile <> "" Then
                ...
                'MainWindow.SetDefaultFieldProperties()               'lthxy
            End If
    ...
    Public Sub FileSave()
        ...
        If saveSettings.SavedFile <> "" Then
            ...
            _bDocumentDirty = False
            MainWindow.Text = PrintDocumentName
            'AddRecentFile(DocumentFileName)             'lthxy
        End If
    End Sub
        ...
        If saveSettings.SavedFile <> "" Then
            ...
            If _documentStreamType <> saveSettings.SavedStreamType Then
                If saveSettings.SavedStreamType = TXTextControl.StreamType.HTMLFormat OrElse saveSettings.SavedStreamType = TXTextControl.StreamType.PlainText OrElse saveSettings.SavedStreamType = TXTextControl.StreamType.PlainAnsiText Then
                    'If MessageBox.Show(My.Resources.strMsgFileHandler01, "Warning", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) = DialogResult.OK Then     'lthxy
                    If MessageBox.Show("Error Save File as!", "Warning", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) = DialogResult.OK Then
                        ' (Re)set application mode
                        'RibbonMain.SetModes(CByte(FormMain.RibbonApplicationMode.Normal))
                        ...
                        'MainWindow.SetDefaultFieldProperties()
                    End If
                End If
                ...
                Select Case streamType
                    ...
                    Case TXTextControl.StreamType.AdobePDFA
                        If Not TextControl.FontSettings.EmbeddableFontsOnly Then
                            'MessageBox.Show(My.Resources.strMsgFileHandler02, "Exporting to PDF/A not possible", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)    'lthxy
                            MessageBox.Show("Exporting to PDF/A not possible", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                            Return
                        End If
...
#Region "  Recent file handling  "
    ...
    Public Sub InitRecentFilesxy()
        _fileList = My.Settings.Default.MRUFiles
        _nMaxFiles = My.Settings.Default.MRUMaxFiles
        'UpdateMenu()             'lthxy
    End Sub
...
#Region "  Helpers  "
    ...
    Private Sub UpdateMainFormSaveStatus()
        'MainWindow.UpdateSaveStatus()            'lthxy
    End Sub
