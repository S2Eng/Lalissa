...
namespace WCFServicePMDS.BAL.Main
{
    ...
    public class PatientMainBAL : IPatientMainBAL
    {
        ...
        {
            Task t = new PatientMainBAL().loadAll(dFrom);
            t.Wait();
        ...
        public async Task loadAll(DateTime dFrom, Guid IDClient)
        {
            using (WCFServicePMDS.Repository.RepositoryWrapper repoWrapper = new Repository.RepositoryWrapper(IDClient))
            {
                ...
                Task<ConcurrentBag<PatientDTO>> taPatient = new PatientDAL(repoWrapper).getAllAsync();
                Task<ConcurrentBag<AufenthaltDTO>> taAufenthalt = new AufenthaltDAL(repoWrapper).getAllAsync();
                Task<ConcurrentBag<KontaktpersonDTO>> taKontaktperson = new KontaktpersonDAL(repoWrapper).getAllAsync();
                Task<ConcurrentBag<PatientAbwesenheitDTO>> taPatientAbwesenheit = new PatientAbwesenheitDAL(repoWrapper).getAllAsync();
                Task<ConcurrentBag<PatientAerzteDTO>> taPatientAerzte = new PatientAerzteDAL(repoWrapper).getAllAsync();
                Task<ConcurrentBag<PatientEinkommenDTO>> taPatientEinkommen = new PatientEinkommenDAL(repoWrapper).getAllAsync();
                Task<ConcurrentBag<PatientenBemerkungDTO>> taPatientenBemerkung = new PatientenBemerkungDAL(repoWrapper).getAllAsync();
                Task<ConcurrentBag<PatientPflegestufeDTO>> taPatientPflegestufe = new PatientPflegestufeDAL(repoWrapper).getAllAsync();
                Task<ConcurrentBag<RehabilitationDTO>> taRehabilitation = new RehabilitationDAL(repoWrapper).getAllAsync();
                Task<ConcurrentBag<SachwalterDTO>> taSachwalter = new SachwalterDAL(repoWrapper).getAllAsync();
                //Dokumente ?
