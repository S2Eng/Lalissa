...
//                this.panelNoQuery.Height = 0;
                                                    ...
                                                    ref string IDApplication)
        {
            try
            {
                ...
                else if (!ControlFound && SearchForGrid)
                {
                    ...
                    newElementMultiControlSelList.loadData();

                    newElementMultiControlSelList.setEditable(true);
                    //newElementMultiControlSelList.Dock = DockStyle.Fill;
                    newElementMultiControlSelList.IsInUseInparameterList = true;
        ...
        public string doActionMulticontrolsxyxy(eActionElement ActionElement, ref qs2.core.vb.dsAdmin.tblQueriesDefDataTable tAllParametersForReport,
                                            eActionType ActionType, ref string SqlWhere, qs2.ui.print.infoQry infoQryRunPar)
        {
            try
            {
                qs2.print.doParameterForQuery doParameterForQuery1 = new qs2.print.doParameterForQuery();

                qs2.core.vb.dsAdmin.tblQueriesDefRow rQryDefFoundBetweenFrom = null;
                qs2.design.auto.multiControl.ownMultiControl ownMCFoundFrom;
                qs2.core.vb.dsAdmin.tblQueriesDefRow rQryDefFoundBetweenTo = null;
                qs2.design.auto.multiControl.ownMultiControl ownMCFoundTo;

                foreach (Control ContFound in this.panelParameters.Controls)
                {
                    if (ContFound.GetType().Equals(typeof(qs2.design.auto.multiControl.ownMultiControl)))
                    {
                        qs2.design.auto.multiControl.ownMultiControl ownMCFound = (qs2.design.auto.multiControl.ownMultiControl)ContFound;
                        if (ownMCFound.Visible)
                        {  
                            if (!ownMCFound.isParameterHeader)
                            {
                                ownMCFound.ownMCTxt1.getSelectedText(ownMCFound, false );
                                //dsAdmin.tblQueriesDefRow rQryConditionUI = doParameterForQuery1.getConditionFromCopyUI(infoQryRunPar.dsConditionsForQuery, ownMCFound.ParIDGuid);

                                qs2.core.vb.dsAdmin.tblQueriesDefRow rQryDefFound = null;
                                if (ActionType == eActionType.WriteSqlWhere)
                                {
                                    rQryDefFound = this.getQueryRowForParameterControlxyxy(ref ownMCFound.ParIDGuid, ref tAllParametersForReport, false);
                                }
                                else
                                {
                                    rQryDefFound = this.getQueryRowForParameterControlxyxy(ref ownMCFound.ParIDGuid, ref tAllParametersForReport, false);
                                ...
                                //if (!(bool)rQryConditionUI[qs2.core.generic.columnRemoved])
                                if (rQryDefFound != null)
                                {
                                    if (ActionType == eActionType.BeetweenRows)
                                    {
                                        if (rQryDefFound[qs2.core.generic.columnStatus].ToString().Trim().ToLower() != qs2.core.generic.eStatusTmp.Done.ToString().Trim().ToLower())
                                        {
                                            if (rQryDefFoundBetweenFrom == null && rQryDefFound.Condition.Trim().ToLower().Equals(qs2.core.sqlTxt.between.Trim().ToLower()))
                                            {
                                                rQryDefFound[qs2.core.generic.columnValueTranslated] = ownMCFound.ownMCTxt1.SelectedText;
                                                rQryDefFoundBetweenFrom = rQryDefFound;
                                                ownMCFoundFrom = ownMCFound;
                                                string CombinationTranslated = qs2.core.language.sqlLanguage.getRes(rQryDefFoundBetweenFrom.Condition.Trim());
                                                if (CombinationTranslated.Trim() == "")
                                                {
                                                    CombinationTranslated = rQryDefFoundBetweenFrom.Condition;
                                                }
                                                rQryDefFoundBetweenFrom[qs2.core.generic.columnParameterLabelTranslated] = ownMCFound.ownMCTxt1.TextTranslated;
                                                rQryDefFoundBetweenFrom[qs2.core.generic.columnParameterCombinationTranslated] = CombinationTranslated;
                                                rQryDefFoundBetweenFrom[qs2.core.generic.columnValueTranslated] = ownMCFoundFrom.ownMCTxt1.SelectedText;
                                                rQryDefFoundBetweenFrom[qs2.core.generic.columnOrderBy] = ownMCFoundFrom.OwnOrderLineNr;
                                                rQryDefFound[qs2.core.generic.columnStatus] = qs2.core.generic.eStatusTmp.Done.ToString();
                                                ownMCFound.TranslatedValueFound = true;
                                            }
                                            if (rQryDefFoundBetweenFrom != null && rQryDefFound.Condition.Trim().ToLower().Equals(qs2.core.sqlTxt.between.Trim().ToLower()) &&
                                                    rQryDefFound[qs2.core.generic.columnStatus] != qs2.core.generic.eStatusTmp.Done.ToString())
                                            {
                                                rQryDefFoundBetweenTo = rQryDefFound;
                                                ownMCFoundTo = ownMCFound;

                                                string IDRessourceTmp = ownMCFound.rSelListQry.IDRessource;
                                                string QryColumnTmp = ownMCFound.rQry.QryColumn;
                                                rQryDefFound[qs2.core.generic.columnParameterLabelTranslated] = ownMCFound.ownMCTxt1.TextTranslated;
                                                string CombinationTranslated = qs2.core.language.sqlLanguage.getRes(rQryDefFoundBetweenTo.Condition.Trim());
                                                if (CombinationTranslated.Trim() == "")
                                                {
                                                    CombinationTranslated = rQryDefFoundBetweenTo.Condition;
                                                }
                                                string AndTranslated = qs2.core.language.sqlLanguage.getRes(qs2.core.sqlTxt.and.Trim());
                                                if (AndTranslated.Trim() == "")
                                                {
                                                    AndTranslated = qs2.core.sqlTxt.and.Trim();
                                                }

                                                rQryDefFoundBetweenTo[qs2.core.generic.columnParameterCombinationTranslated] = CombinationTranslated;
                                                rQryDefFoundBetweenTo[qs2.core.generic.columnValueTranslated] = ownMCFoundTo.ownMCTxt1.SelectedText;
                                                rQryDefFoundBetweenTo[qs2.core.generic.columnOrderBy] = ownMCFoundTo.OwnOrderLineNr;
                                                rQryDefFoundBetweenTo[qs2.core.generic.columnParameterForReport] = ownMCFound.ownMCTxt1.TextTranslated.Trim() + " " + CombinationTranslated.Trim() + " " +
                                                                                                            rQryDefFoundBetweenFrom[qs2.core.generic.columnValueTranslated].ToString().Trim() + " " + AndTranslated.Trim() + " " + rQryDefFoundBetweenTo[qs2.core.generic.columnValueTranslated].ToString().Trim() + "";
                                                rQryDefFoundBetweenFrom[qs2.core.generic.columnParameterForReport] = rQryDefFoundBetweenTo[qs2.core.generic.columnParameterForReport];

                                                //rQryDefFoundBetweenFrom = null;
                                                //rQryDefFoundBetweenTo = null;
                                                rQryDefFound[qs2.core.generic.columnStatus] = qs2.core.generic.eStatusTmp.ForDelete.ToString();
                                                ownMCFound.TranslatedValueFound = true;
                                    ...
                                    else if (ActionType == eActionType.NormalRows)
                                    {
                                        if (rQryDefFound[qs2.core.generic.columnStatus] != qs2.core.generic.eStatusTmp.Done.ToString() &&
                                            rQryDefFound[qs2.core.generic.columnStatus] != qs2.core.generic.eStatusTmp.ForDelete.ToString())
                                        {
                                            string IDRessourceTmp = ownMCFound.rSelListQry.IDRessource;
                                            string QryColumnTmp = ownMCFound.rQry.QryColumn;

                                            string CombinationTranslated = qs2.core.language.sqlLanguage.getRes(rQryDefFound.Condition);
                                            if (CombinationTranslated.Trim() == "")
                                            {
                                                CombinationTranslated = rQryDefFound.Condition;
                                            }

                                            rQryDefFound[qs2.core.generic.columnParameterLabelTranslated] = ownMCFound.ownMCTxt1.TextTranslated;
                                            rQryDefFound[qs2.core.generic.columnParameterCombinationTranslated] = CombinationTranslated;
                                            rQryDefFound[qs2.core.generic.columnValueTranslated] = ownMCFound.ownMCTxt1.SelectedText;
                                            rQryDefFound[qs2.core.generic.columnOrderBy] = ownMCFound.OwnOrderLineNr;

                                            rQryDefFound[qs2.core.generic.columnParameterForReport] = ownMCFound.ownMCTxt1.TextTranslated.Trim() + " " + CombinationTranslated.Trim() + " " +
                                                                                                                                ownMCFound.ownMCTxt1.SelectedText.Trim();
                                            rQryDefFound[qs2.core.generic.columnStatus] = qs2.core.generic.eStatusTmp.Done.ToString();
                                        }
                                        ownMCFound.TranslatedValueFound = true;
                                    }
                                    else if (ActionType == eActionType.DeleteRows)
                                    {
                                        if (rQryDefFound[qs2.core.generic.columnStatus] == qs2.core.generic.eStatusTmp.ForDelete.ToString())
                                        {
                                            rQryDefFound.Delete();
                                            tAllParametersForReport.AcceptChanges();
                                        }
                                    }
                                    else if (ActionType == eActionType.GetSqlWhere)
                                    {
                                        if (rQryDefFound != null)
                                        {
                                            rQryDefFound[qs2.core.generic.columnParameterCombinationTranslated] = ownMCFound.ownMCTxt1.TextCombinationTranslated.Trim();
                                            rQryDefFound[qs2.core.generic.columnParameterCombinationEndTranslated] = ownMCFound.ownMCTxt1.TextCombinationEndTranslated.Trim();
                                            SqlWhere += rQryDefFound[qs2.core.generic.columnParameterCombinationTranslated] + " " + rQryDefFound[qs2.core.generic.columnParameterForReport].ToString().Trim() + " " + rQryDefFound[qs2.core.generic.columnParameterCombinationEndTranslated] + "";
                                            if (rQryDefFound.Typ.Trim().ToLower() == qs2.core.Enums.eTypQueryDef.InputParameters.ToString().Trim().ToLower() &&
                                                    !rQryDefFound.FunctionPar)
                                            {
                                                SqlWhere += "\r\n";
                                            }
                                        }
                                    }
                                    else if (ActionType == eActionType.WriteSqlWhere)
                                    {
                                        if (rQryDefFound != null)
                                        {
                                            rQryDefFound[qs2.core.generic.columnParameterSqlWhereUI] = SqlWhere;
                                        }
                                    }
                                }
                            } 
                ...
                if (ActionType == eActionType.NormalRows)
                {
                    foreach (Control ContFound in this.panelParameters.Controls)
                    {
                        if (ContFound.GetType().Equals(typeof(qs2.design.auto.multiControl.ownMultiControl)))
                        {
                            ...
                            if (ownMCFound.Visible)
                            {
                                if (!ownMCFound.isParameterHeader)
                                {
                                    if (!ownMCFound.TranslatedValueFound)
                                    {
                                        //throw new Exception("contQryRunPar.doParameterMulticontrol: for Parameter '" + ownMCFound.ownMCTxt1.TextTranslated + "' in Query not translatedValue found in Multicontrol!");
            ...
            {
                throw new Exception("contQryRunPar.doParameterMulticontrol: " + ex.ToString());
            }
        }

        public qs2.core.vb.dsAdmin.tblQueriesDefRow getQueryRowForParameterControlxyxy(ref System.Guid ParIDGuidToSearch,
                                                                            ref qs2.core.vb.dsAdmin.tblQueriesDefDataTable tAllParametersForReport,
                                                                            bool DoExceptionIfNotFound)
        {
            try
            {
                foreach (qs2.core.vb.dsAdmin.tblQueriesDefRow rQryDefFound in tAllParametersForReport)
                {
                    if (rQryDefFound.IDGuid.Equals(ParIDGuidToSearch))
                    {
                        return rQryDefFound;
                    }               
                }
                if (DoExceptionIfNotFound)
                {
                    throw new Exception("contQryRunPar.getQueryForParameterControl: Control not found !");
                }
                return null;
            }
            catch (Exception ex)
            {
                throw new Exception("contQryRunPar.getQueryRowForParameterControlxyxy: " + ex.ToString());
            }
        }
