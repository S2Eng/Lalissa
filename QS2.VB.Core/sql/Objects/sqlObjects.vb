...
Public Class sqlObjects
    ...
    Public Function checkUsrSupervisor(Supervisor As Boolean, Admin2 As Boolean, IDParticipant As String, Superadmin As Boolean,
                                       ByRef IDGuidObjBack As Guid, ByRef IDObjectBack As Integer) As Boolean
        Try
            Dim UserLastNameTmp As String = ""
            Dim UserFirstNameTmp As String = "A."
            If Supervisor Then
                UserLastNameTmp = sqlObjects.userName_Supervisor.Trim()
            ElseIf Admin2 Then
                UserLastNameTmp = sqlObjects.userName_Admin.Trim()
            ElseIf Superadmin Then
                UserLastNameTmp = sqlObjects.userName_Superadmin.Trim()
            Else
                Throw New Exception("checkUsrSupervisor: Error not Supervisor and not Admin!")
            End If
            ...
            Dim ds As New dsObjects()
            If Supervisor Then
                sqlObjects1.getObject(0, ds, sqlObjects.eTypSelObj.UserName, sqlObjects.eTypObj.none, "", UserLastNameTmp)
            Else
                sqlObjects1.getObject(0, ds, sqlObjects.eTypSelObj.UserNameParticipant, sqlObjects.eTypObj.none, "", UserLastNameTmp, False, Nothing, IDParticipant)
            End If
            If ds.tblObject.Rows.Count > 1 Then
                Throw New Exception("ds.tblObject.Rows.Count > 1 for User '" + UserLastNameTmp.Trim() + "'!")
            End If
            If ds.tblObject.Rows.Count = 0 Or ds.tblObject.Rows.Count = 1 Then
                Dim rObj As dsObjects.tblObjectRow
                If ds.tblObject.Rows.Count = 0 Then
                    rObj = Me.getNewRowObject(ds)
                    rObj.ExtID = System.Guid.NewGuid().ToString()
                    If Admin2 Then
                        Dim Encryption1 As New qs2.license.core.Encryption()
                        rObj.Password = Encryption1.StringEncrypt("Admin", qs2.license.core.Encryption.keyForEncryptingStrings)
                    ElseIf Superadmin Then
                        Dim Encryption1 As New qs2.license.core.Encryption()
                        rObj.Password = Encryption1.StringEncrypt("Superadmin", qs2.license.core.Encryption.keyForEncryptingStrings)
                    End If
                ElseIf ds.tblObject.Rows.Count = 1 Then
                    rObj = ds.tblObject.Rows(0)
                End If
                rObj.LastName = UserLastNameTmp.Trim()
                rObj.FirstName = UserFirstNameTmp.Trim()
                rObj.NameCombination = rObj.LastName + "" + rObj.FirstName
                rObj.IsUser = True
                rObj.isAdmin = True
                rObj.UserShort = UserLastNameTmp.Trim()
                rObj.UserName = UserLastNameTmp.Trim()
                If Admin2 Then
                    rObj.IDParticipant = IDParticipant
                ElseIf Superadmin Then
                    rObj.IDParticipant = IDParticipant
                End If
                'rObj.IDParticipant = qs2.core.license.doLicense.rParticipant.IDParticipant.Trim()

                If Supervisor Then
                    Dim Encryption1 As New qs2.license.core.Encryption()
                    rObj.Password = Encryption1.StringEncrypt(qs2.license.core.Encryption.keyForEncryptingStrings, qs2.license.core.Encryption.keyForEncryptingStrings)
                Else
                    rObj.UserNameDomain = rObj.UserNameDomain.Trim()
                End If

                sqlObjects1.daObject.Update(ds.tblObject)
                If Superadmin Then
                    Me.checkAppsExistsForUser(rObj.IDGuid)
                End If

                IDGuidObjBack = rObj.IDGuid
                IDObjectBack = rObj.ID
            End If

        Catch ex As Exception
            Throw New Exception("sqlObjects.checkUsrSupervisor: " + vbNewLine + vbNewLine + ex.ToString())
        End Try
    End Function
    Public Function checkAppsExistsForUser(ByRef IDGuidObject As Guid) As Boolean
        Try
            ...
            With db
                ...
                Dim tObjectApplications As IQueryable(Of qs2.db.Entities.tblObjectApplications) = b.getProductsForUser(IDGuidObject, db)
                Dim bCARDIACExistsForuser As Boolean = False
                Dim bVASCULARExistsForUser As Boolean = False
                Dim bNCExistsForUser As Boolean = False
                Dim bQTHExistsForUser As Boolean = False

                For Each rAppInDB As qs2.db.Entities.tblObjectApplications In tObjectApplications
                    If qs2.core.generic.sEquals(rAppInDB.IDApplication, license.doLicense.eApp.CARDIAC) Then
                        bCARDIACExistsForuser = True
                    ElseIf qs2.core.generic.sEquals(rAppInDB.IDApplication, license.doLicense.eApp.VASCULAR) Then
                        bVASCULARExistsForUser = True
                    ElseIf qs2.core.generic.sEquals(rAppInDB.IDApplication, license.doLicense.eApp.QTH) Then
                        bQTHExistsForUser = True
                    ElseIf qs2.core.generic.sEquals(rAppInDB.IDApplication, license.doLicense.eApp.NC) Then
                        bNCExistsForUser = True
                    End If
                Next

                Dim bAppAdded As Boolean = False
                If Not bCARDIACExistsForuser Then
                    Dim rObjectApplications As New qs2.db.Entities.tblObjectApplications()
                    rObjectApplications.ID = System.Guid.NewGuid()
                    rObjectApplications.IDObjectGuid = IDGuidObject
                    rObjectApplications.IDApplication = qs2.core.license.doLicense.eApp.CARDIAC.ToString()
                    db.tblObjectApplications.Add(rObjectApplications)
                    bAppAdded = True
                End If

                If Not bVASCULARExistsForUser Then
                    Dim rObjectApplications As New qs2.db.Entities.tblObjectApplications()
                    rObjectApplications.ID = System.Guid.NewGuid()
                    rObjectApplications.IDObjectGuid = IDGuidObject
                    rObjectApplications.IDApplication = qs2.core.license.doLicense.eApp.VASCULAR.ToString()
                    db.tblObjectApplications.Add(rObjectApplications)
                    bAppAdded = True
                End If

                If Not bNCExistsForUser Then
                    Dim rObjectApplications As New qs2.db.Entities.tblObjectApplications()
                    rObjectApplications.ID = System.Guid.NewGuid()
                    rObjectApplications.IDObjectGuid = IDGuidObject
                    rObjectApplications.IDApplication = qs2.core.license.doLicense.eApp.NC.ToString()
                    db.tblObjectApplications.Add(rObjectApplications)
                    bAppAdded = True
                End If

                If Not bQTHExistsForUser Then
                    Dim rObjectApplications As New qs2.db.Entities.tblObjectApplications()
                    rObjectApplications.ID = System.Guid.NewGuid()
                    rObjectApplications.IDObjectGuid = IDGuidObject
                    rObjectApplications.IDApplication = qs2.core.license.doLicense.eApp.QTH.ToString()
                    db.tblObjectApplications.Add(rObjectApplications)
                    bAppAdded = True
                End If
        ...
        Catch ex As Exception
            Throw New Exception("sqlObjects.checkAppsExistsForUser: " + vbNewLine + vbNewLine + ex.ToString())
        End Try
    End Function
