...
namespace PMDS.Calc.UI.Admin.abwÜbersp
{
    public class abwÜberspSitemap
    {
        ...
        public PMDS.UI.Sitemap.UIFct sitemap { get; set; } = new PMDS.UI.Sitemap.UIFct();
        ...
        public void searchAbw(DateTime von, DateTime bis, ref  dsAbwÜbersp ds, int typAuswÜbersp, bool händischJN,System.Guid IDKlinik, int MinTageAbwesenheit)
        {
            ...
            using (dsPatientStation.PatientDataTable tRet = new dsPatientStation.PatientDataTable())
            {
                ...
                foreach (dsPatientStation.PatientRow rKlient in dtAllPat.Rows)
                {
                    ...
                    {
                        List<Guid> lAufenthaltIDS = (from a in db.Aufenthalt
                                                     where a.IDPatient == rKlient.ID
                                                     select a.ID).ToList();

                        var lUrlaubVerlauf = (from uv in db.UrlaubVerlauf
                                              where ((uv.StartDatum >= von && uv.StartDatum <= bis) || (uv.EndeDatum >= von && uv.EndeDatum <= bis)) &&
                                                lAufenthaltIDS.Contains((Guid)uv.IDAufenthalt)
                                              orderby uv.StartDatum
                                              select uv
                                              ).ToList();

                        //dsUrlaubVerlauf.UrlaubVerlaufDataTable dtUrlaub = PMDS.BusinessLogic.UrlaubVerlauf.alleUrlaube(rKlient.ID, von, bis);
                        foreach (var rUrlaub in lUrlaubVerlauf)
                        {
                            ...
                            rNew.Von = (DateTime)rUrlaub.StartDatum;
            ...
            try
            {
                if (!String.IsNullOrWhiteSpace(errStartDat))
