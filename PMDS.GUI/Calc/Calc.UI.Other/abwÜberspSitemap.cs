...
namespace PMDS.Calc.UI.Admin.abwÜbersp
{
    public class abwÜberspSitemap
    {
        ...
        {
            string errStartDat = "";
            PMDS.Data.Patient.dsPatientStation.PatientDataTable tRet = new PMDS.Data.Patient.dsPatientStation.PatientDataTable();
            PMDS.Data.Patient.dsPatientStation.PatientDataTable dtAllPat = sitemap.loadAllKlientenProdHist(true, PMDS.Global.ENV.IDKlinik, tRet);
            foreach (PMDS.Data.Patient.dsPatientStation.PatientRow rKlient in dtAllPat.Rows)
            {
                PMDS.Data.Patient.dsUrlaubVerlauf.UrlaubVerlaufDataTable dtUrlaub = PMDS.BusinessLogic.UrlaubVerlauf.alleUrlaube(rKlient.ID, von, bis );
                foreach (PMDS.Data.Patient.dsUrlaubVerlauf.UrlaubVerlaufRow rUrlaub in dtUrlaub)
                {
                    ...
                    else if (dtBereitsÜbersp.Count == 1)                //r = in Abrechung eingetragene Daten, rNew = in der Pflege erfasste Daten
                    {
