...
namespace PMDS.GUI.GUI.Main
{
    ...
    public partial class frmSys : Form
    {
        ...
        private void btnMigrateOldMessages_Click(object sender, EventArgs e)
        {
            try
            {
                this.Cursor = Cursors.WaitCursor;
                ...
                using (PMDS.db.Entities.ERModellPMDSEntities db = PMDSBusiness.getDBContext())
                {
                    b.sys_MigrateOldMessages(db, this.lblInfo, ref lUpdated, ref lErrors);
                }

                string sMsgBox = "Erfolgreich beendet!" + "\r\n" + "\r\n" + "Anzahl Nachrichten migriert: " + lUpdated.Count().ToString() + "\r\n" + "Anzahl Errors: " + lErrors.Count().ToString() + "";
                System.Windows.Forms.MessageBox.Show(sMsgBox, "Nachrichten migrieren", MessageBoxButtons.OK);
